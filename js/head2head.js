...
const playerBioUrl = `../api/get_player_info.php`;
const playerRadialUrl = `../api/player_radials.php`;
const playerRadarUrl = `../api/player_radar.php`;
const playerRatingUrl = `../api/player_rating.php`;
const playerSearchUrl = `../api/player_search.php`;
const playerYearsUrl = `../api/player_years.php`;
const statIdsList = `../api/get_abbrev_and_stat.php`;
const playerEventsUrl = `../api/player_events.php`;
...
playerOneInput.addEventListener("input", async () => {
  ...
  let url = query ? playerSearchUrl + `?query=` + query : "";
...
async function displayPlayerRadials() {
  let yearUrl =
    pdgaNumOne && !pdgaNumTwo
      ? playerYearsUrl + "?pdga_number=" + pdgaNumOne
      : pdgaNumOne && pdgaNumTwo
      ? playerYearsUrl + "?pdga_number=" + pdgaNumOne
      : !pdgaNumOne && pdgaNumTwo
      ? playerYearsUrl + "?pdga_number=" + pdgaNumTwo
      : "";
  ...
  async function drawRadials(year) {
    let url = pdgaNumOne ? playerRadialUrl + `?pdga_number=${pdgaNumOne}` : "";

    url = year ? url + "&year=" + year : url;

    let url2 = pdgaNumTwo ? playerRadialUrl + `?pdga_number=${pdgaNumTwo}` : "";

    url2 = year ? url2 + "&year=" + year : url2;
...
async function displayPlayerBio(
  ...
  const h2hCareerProfileDropdown = document.getElementById(
    "h2h_careerProfile_dropdown"
  );
  ...
  async function drawCareerProfile(isLast12) {
    ...
    let url = pdgaNum ? playerBioUrl + `?pdga_number=${pdgaNum}` : "";

    url = isLast12 ? url + "&is_last_12_months=" + isLast12 : url;
    ...
    if (data) {
      ...
      document.getElementById(
        pdgaNumElement
      ).innerHTML = `#${data.player.pdga_number}`;
      document.getElementById(
        divElement
      ).innerHTML = `${data.player.division} Division`;
      document.getElementById(
        memberElement
      ).innerHTML = `member since <strong>${data.player.member_since}</strong>`;
     ...
     */
    let url2 = pdgaNum2 ? playerBioUrl + `?pdga_number=${pdgaNum2}` : "";

    url2 = isLast12 ? url2 + "&is_last_12_months=" + isLast12 : url2;
    ...
    if (data2) {
      ...
      document.getElementById(
        picElement2
      ).src = `./../assets/${data2.player.pdga_number}.jpg`;
      ...
      document.getElementById(
        pdgaNumElement2
      ).innerHTML = `#${data2.player.pdga_number}`;
      document.getElementById(
        divElement2
      ).innerHTML = `${data2.player.division} Division`;
      document.getElementById(
        memberElement2
      ).innerHTML = `member since <strong>${data2.player.member_since}</strong>`;
    ...
    if (data && data2) {
      ...
      if (parseInt(data.player.top_tens) > parseInt(data2.player.top_tens)) {
        ...
        parseInt(data.player.top_tens) < parseInt(data2.player.top_tens)
      ) {
      ...
      if (parseInt(data.player.podiums) > parseInt(data2.player.podiums)) {
        ...
        parseInt(data.player.podiums) < parseInt(data2.player.podiums)
      ) {
      ...
      if (parseInt(data.player.earnings) > parseInt(data2.player.earnings)) {
        ...
        parseInt(data.player.earnings) < parseInt(data2.player.earnings)
      ) {
      ...
      if (
        parseInt(data.player.avg_rating) > parseInt(data2.player.avg_rating)
      ) {
      ...
      if (
        parseInt(data.player.total_events) > parseInt(data2.player.total_events)
      ) {
      ...
      if (parseInt(data.player.avg_place) < parseInt(data2.player.avg_place)) {
        ...
        parseInt(data.player.avg_place) > parseInt(data2.player.avg_place)
      ) {
      ...
      if (
        parseInt(data.player.avg_strokes_per_event) <
        parseInt(data2.player.avg_strokes_per_event)
      ) {
        document.getElementById(avgStrokesPerEventElement).style.color =
          "#A6FFA6";
        document.getElementById(avgStrokesPerEventElement2).style.color =
          "#FF3E3E";
      } else if (
        parseInt(data.player.avg_strokes_per_event) >
        parseInt(data2.player.avg_strokes_per_event)
...
async function displayPlayerRadars() {
  ...
  async function drawRadars(year, eventId, valuesIds) {
    let url = pdgaNumOne ? playerRadarUrl + `?pdga_number=${pdgaNumOne}` : "";

    url = year ? url + "&year=" + year : url;

    url = eventId ? url + "&event=" + eventId : url;

    let url2 = pdgaNumTwo ? playerRadarUrl + `?pdga_number=${pdgaNumTwo}` : "";

    url2 = year ? url2 + "&year=" + year : url2;

    url2 = eventId ? url2 + "&event=" + eventId : url2;
  ...
  async function getEventsFromYear(year) {
    let eventsUrl =
      pdgaNumOne && !pdgaNumTwo
        ? playerEventsUrl + "?pdga_number=" + pdgaNumOne
        : pdgaNumOne && pdgaNumTwo
        ? playerEventsUrl + "?pdga_number=" + pdgaNumOne
        : !pdgaNumOne && pdgaNumTwo
        ? playerEventsUrl + "?pdga_number=" + pdgaNumTwo
        : "";
...
async function displayPlayerRatingLine() {
   ...
   */
  let url = pdgaNumOne ? playerRatingUrl + `?pdga_number=${pdgaNumOne}` : "";

  let url2 = pdgaNumTwo ? playerRatingUrl + `?pdga_number=${pdgaNumTwo}` : "";
...
function loadShow() {
  ...
  for (var i = active + 1; i < items.length; i++) {
    stt++;
    items[i].style.transform = `translate(${120 * stt}px) scale(${
      1 - 0.2 * stt
...
let initialPlayerSuggestion = document.getElementById(
  "initialPlayer_suggestion"
);
